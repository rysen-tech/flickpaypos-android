                    ...
                    synchronized(GLOBAL_TRANSPORT_LOCK) transportSync@{
                        ...
                        when (print.mode) {
                            PrinterMode.BLUETOOTH -> {
                                val adapter = BluetoothAdapter.getDefaultAdapter() ?: return@transportSync
        ...
        repeat(USB_TARGET_RETRIES) { attempt ->
            ...
            val ok = try {
                if (!connection.claimInterface(target.iface, true)) {
                    ...
                    // Some USB thermal units need a tiny wake/init transaction after interface claim.
