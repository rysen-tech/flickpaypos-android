...
object AppUpdateManager {
    ...
    private fun launchInstaller(activity: Activity, apkFile: File): InstallLaunchResult {
        ...
        prepareForSystemInstaller(activity, reason = "apk_install")
    ...
    private fun openUnknownSourcesSettings(activity: Activity) {
        prepareForSystemInstaller(activity, reason = "unknown_sources")
    ...
    private fun prepareForSystemInstaller(activity: Activity, reason: String) {
        val appContext = activity.applicationContext
        val kioskEnabled = runCatching { AppSettings(activity).isHomeLauncherEnabled() }.getOrDefault(false)
        if (!kioskEnabled) return
        runCatching {
            KioskModeManager.enforce(activity, enabledInSettings = false, reason = "update_$reason")
        }.onSuccess { state ->
            AppRuntimeLog.i(
                appContext,
                "AppUpdate",
                "Temporarily relaxed kiosk for update ($reason): owner=${state.deviceOwner} lockTask=${state.inLockTask}"
            )
        }.onFailure { err ->
            AppRuntimeLog.e(appContext, "AppUpdate", "Failed to relax kiosk for update ($reason)", err)
        }
    }
