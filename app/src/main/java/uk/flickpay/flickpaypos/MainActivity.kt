...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AppSettings(this).syncComponentStates()
        applyRotationLockPreference()
        enableImmersiveMode()
        ...
        updateCustomerDisplayFromPrefs()

        registerUsbPermissionReceiver()
        requestUsbPermissionsForAttachedPrinters()
        startPosBackgroundService()
        configureWebView()
        loadConfiguredUrlWhenProxyReady()
    }
    ...
    private fun configureWebView() {
        ...
        webView.webViewClient = object : WebViewClient() {
                ...
                view?.evaluateJavascript(
                    ...
                    (() => {
                      try {
                        ...
                        localStorage.setItem("hw_proxy_url", "https://127.0.0.1:8070");
            ...
            override fun onReceivedError(
                ...
                val cachedPosUrl = getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                    .getString(KEY_POS_URL, null)
                    ?.trim()
                    .orEmpty()
                if (cachedPosUrl.isBlank()) {
                    hideLaunchOverlay()
                    return
                }

                val current = view?.url?.trim().orEmpty()
                if (current.equals(cachedPosUrl, ignoreCase = true)) {
                    hideLaunchOverlay()
                    return
                }
                ...
                view?.post {
                    runCatching { view.loadUrl(cachedPosUrl) }
                }
            }
        ...
        view?.evaluateJavascript(
            ...
            (() => {
              try {
                ...
                const helloUrl = "https://127.0.0.1:8070/hw_proxy/hello";
                const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
                const ping = async () => {
                  for (let i = 0; i < 6; i++) {
                    try {
                      const res = await fetch(helloUrl, { method: "GET", cache: "no-store" });
    ...
    private fun buildPosUrl(
        ...
        val accountHost = normalizeAccountHost(accountHostRaw)
    ...
    private fun handlePosCommand(uri: Uri): Boolean {
