...
class PosBackgroundService : Service() {
    ...
    private val tag = "FlickpayPOSService"
    ...
    override fun onCreate() {
        ...
        runCatching {
            ...
            scheduleStartupWarmupRetries()
            createChannel()
            startForeground(NOTIFICATION_ID, buildNotification())
            startServer()
        }.onFailure { err ->
            AppRuntimeLog.e(applicationContext, tag, "Service startup failed", err)
            runCatching { servers.forEach { it.stop() } }
            servers.clear()
    ...
    private fun startServer() {
        ...
        for (route in appSettings.getPrinterRoutes()) {
            ...
            // Android WebView is configured for localhost cleartext + mixed content.
            // Running plain local proxy avoids TLS keystore compatibility failures
            // on some Android builds and removes startup delay.
            val server = runCatching {
                HwProxyServer(this, printer, port = port).also {
                    it.start(NanoHTTPD.SOCKET_READ_TIMEOUT, false)
                }
            }.onFailure { err ->
                AppRuntimeLog.e(applicationContext, tag, "Print proxy failed on ${route.key}:$port", err)
            }.getOrNull()
        ...
        if (servers.isEmpty()) {
            AppRuntimeLog.e(applicationContext, tag, "No print proxy server could be started; service stays alive")
        }
    }
    ...
    private fun scheduleStartupWarmupRetries() {
        Thread {
            repeat(4) {
