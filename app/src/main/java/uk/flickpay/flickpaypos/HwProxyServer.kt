...
class HwProxyServer(
    ...
    private fun handleDefaultPrinterAction(params: JSONObject?): Boolean {
        ...
        return when (receipt) {
            is JSONObject -> {
                ...
                val template = receipt.optString("template").lowercase(Locale.ROOT)
                ...
                val nativeTemplate = template == "android_pos_native_v1" || mode.contains("android_native")
                val payloadObject = parsePayloadObject(receipt.opt("data"))
                val payload = receipt.optString("data")

                if (nativeTemplate && payloadObject != null) {
                    val rendered = AndroidPosReceiptFormatter.render(payloadObject)
                    if (rendered.isBlank()) {
                        false
                    } else {
                        printer.printEscposText(rendered)
                    }
    ...
    private fun parsePayloadObject(payload: Any?): JSONObject? {
        return when (payload) {
